...
// =======================================
// PARTE VISUAL DA CALCULADORA NO TERMINAL
// =======================================
void calculadora(){
    //CALCULADORA
    printf("****     *     *       ****   *    *   *         *     ****     ***    ***      *    \n");
...
int eh_inteiro(float a, float b){
    if(a == (int)a && b == (int)b){
        return 1; 
    }
    else{
...
void combinar_nos(Leitura* operador_node) {
    ...
    if(idx ==  4){
        int answer = eh_inteiro(a, b);
        //Se a e b for inteiro calculo por bit a bit
        if(answer == 1){
...
float potenciaint(float a, float b){
    return powf(a,b);
}

void salvar_historico(const char* expressao, double resultado) {
    FILE* f = fopen("historico.txt", "a"); // abre no modo append
    if (f == NULL) {
...
void mostrar_historico() {
    ...
    if (f == NULL) {
        printf("Ainda não há histórico salvo.\n");
        return;
    }
...
void parenteses(char* expressao){ 
    char *sub = malloc(32);
    char buffer[32];   // Para sprintf (transforma float em string)
    int inicio, fim;
    float resultado;
    int flag = 0; 
    ...
    while (1) {
        // Encontrar o último '('
        //Analisa se eh parenteses trigonometrico ou nao
        inicio = -1;
        for (int i = 0; expressao[i] != '\0'; i++) {
            if (expressao[i] == '(' && (i==0 || !isalpha(expressao[i-1]))){
                inicio = i;
            }
            if(i>0){
                if(expressao[i] == '(' && (isalpha(expressao[i-1]))){
                    flag = 1; 
                }
            }
        }

        // Nenhum '(' encontrado = acabou
        if (inicio == -1)
            break;

        // Encontrar o ')' correspondente
        for (int i = 0; expressao[i] != '\0'; i++) {
            if (expressao[i] == ')' && (flag == 0)){
                fim = i;
            }
            if(i>0){
                if(expressao[i] == '(' && (flag == 1)){
                    flag = 0; 
                }
            }
        }
        ...
        if (expressao[fim] == '\0'){
            printf("Erro: parênteses incorretos.\n");
            free(sub);
            return;
        }

        // Copiar a subexpressão (conteúdo entre ( ... ))
        int t = 0;
        ...
        resultado = processar_expressao(sub);

        // Transformar float em string
        sprintf(buffer, "%.2f", resultado);

        // ======== SUBSTITUIR ( ... ) PELO NÚMERO =========
        // Escrever o número no lugar do '('
        int len_num = strlen(buffer);
        for (int i = 0; i < len_num; i++)
            expressao[inicio + i] = buffer[i];

        // Mover o resto da string para trás 
        int idx = 0;
        int start = inicio + len_num;
        int src = fim + 1;

        while (expressao[src] != '\0') {
            expressao[start + idx] = expressao[src];
            idx++;
            src++;
        }

        expressao[start + idx] = '\0';
        // Apaga qualquer coisa após o final da cópia
        int pos = start + idx;
    ...
    // Sem parênteses restantes = avaliar expressão final
    resultado = processar_expressao(expressao);

    printf("Resultado: %.4f\n", resultado);

    salvar_historico(expressao, resultado);

    free(sub);
}

float processar_expressao(char* expressao) {
    ...
    char trigo[4];                      // String para analisar se eh uma funcao trigonometrica
    char valor[32];                      // String que armazena o valor a ser processado na funcao trigonometrica
...
int modo(){
    ...
    printf("Suporte para: +, -, *, /, !, ^, () e trigonometria\n");
    printf("Para cosseno -> cos(x)\nPara seno -> sen(x) ou sin(x)\nPara tangente -> tan(x)\n");

    // ========== LOOP PRINCIPAL DO PROGRAMA ==========
    while (1) {
        // Pergunta qual modo o usuário deseja
        printf("\n-> Fibonacci(F)");
        printf("\n-> Expressao numerica(E)");
        printf("\n-> Ver Histórico(H)");
        printf("\n-> Sair(S)");
